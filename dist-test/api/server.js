...
exports.searchEngine = exports.fastify = exports.isInitialized = void 0;
exports.initializeServer = initializeServerWithTracking;
...
fastify.setErrorHandler((error, request, reply) => {
    ...
    const traceId = request.headers['x-trace-id'] || (0, uuid_1.v4)();
...
const searchEngine = new search_engine_js_1.LensSearchEngine();
...
fastify.get('/health', async (request, reply) => {
    const span = tracer_js_1.LensTracer.createChildSpan('health_check');
    const startTime = Date.now();
    try {
        const health = await searchEngine.getHealthStatus();
        const latency = Date.now() - startTime;
        // Check SLA compliance (5ms target)
        if (latency > config_js_1.PRODUCTION_CONFIG.api_limits.rate_limit_per_sec) {
        ...
        api_js_1.HealthResponseSchema.parse(response);
        return response;
    }
    catch (error) {
...
fastify.post('/search', async (request, reply) => {
    const span = tracer_js_1.LensTracer.createChildSpan('api_search');
    const startTime = Date.now();
    try {
        // Validate request
        const searchRequest = api_js_1.SearchRequestSchema.parse(request.body);
        const traceId = request.headers['x-trace-id'] || (0, uuid_1.v4)();
        span.setAttributes({
        ...
        // Validate response
        api_js_1.SearchResponseSchema.parse(response);
        span.setAttributes({
            success: true,
        ...
        span.recordException(error);
        span.setAttributes({ success: false, error: errorMsg });
        fastify.log.error(`Search failed: ${errorMsg}`, { traceId });
...
fastify.post('/struct', async (request, reply) => {
    const span = tracer_js_1.LensTracer.createChildSpan('api_struct');
    const startTime = Date.now();
    try {
        // Validate request
        const structRequest = api_js_1.StructRequestSchema.parse(request.body);
        const traceId = request.headers['x-trace-id'] || (0, uuid_1.v4)();
        span.setAttributes({
...
fastify.post('/symbols/near', async (request, reply) => {
    const span = tracer_js_1.LensTracer.createChildSpan('api_symbols_near');
    const startTime = Date.now();
    try {
        // Validate request
        const symbolsRequest = api_js_1.SymbolsNearRequestSchema.parse(request.body);
        const traceId = request.headers['x-trace-id'] || (0, uuid_1.v4)();
        span.setAttributes({
        ...
        const result = await searchEngine.findSymbolsNear(symbolsRequest.file, symbolsRequest.line, symbolsRequest.radius || 25);
        const totalLatency = Date.now() - startTime;
