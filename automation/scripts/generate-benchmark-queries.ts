 ...
 * 
 * Uses actual corpus content to create realistic search scenarios
 */
...
interface BenchmarkQuery {
  ...
  query: string;
  suite: string;
  intent: 'semantic' | 'identifier' | 'structural' | 'exact';
  language: string;
  expected_file?: string;
  difficulty: 'easy' | 'medium' | 'hard';
}

interface QuerySuite {
  name: string;
...
class BenchmarkQueryGenerator {
  ...
  async generateAllSuites(): Promise<void> {
  ...
  private async readCorpusFiles(): Promise<Array<{path: string, content: string, language: string}>> {
  ...
  private async generateCoirQueries(corpusFiles: Array<{path: string, content: string, language: string}>): Promise<BenchmarkQuery[]> {
  ...
  private async generateSweQueries(corpusFiles: Array<{path: string, content: string, language: string}>): Promise<BenchmarkQuery[]> {
  ...
  private async generateCsnQueries(corpusFiles: Array<{path: string, content: string, language: string}>): Promise<BenchmarkQuery[]> {
  ...
  private async generateCosqaQueries(corpusFiles: Array<{path: string, content: string, language: string}>): Promise<BenchmarkQuery[]> {
  ...
  private convertDocstringToQuery(docstring: string): string | null {
  ...
  private async saveQuerySuite(suiteName: string, queries: BenchmarkQuery[]): Promise<void> {
  ...
  private async generateSummary(suites: QuerySuite[]): Promise<void> {
    const summary = {
  ...
  private getIntentDistribution(queries: BenchmarkQuery[]): Record<string, number> {
  ...
  private getLanguageDistribution(queries: BenchmarkQuery[]): Record<string, number> {
    const distribution: Record<string, number> = {};
    for (const query of queries) {
  ...
  private getDifficultyDistribution(queries: BenchmarkQuery[]): Record<string, number> {
    const distribution: Record<string, number> = {};
    for (const query of queries) {
...
export default BenchmarkQueryGenerator;