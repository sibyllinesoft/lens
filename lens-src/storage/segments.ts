...
import type { MMapSegment, Segment, SegmentType, Shard } from '../types/core.js';
import { LensTracer } from '../telemetry/tracer.js';
import { PRODUCTION_CONFIG } from '../types/config.js';

interface SegmentHeader {
  magic: number;           // 0x4C454E53 ('LENS')
...
export class SegmentStorage {
  private segments: Map<string, MMapSegment> = new Map();
  private readonly basePath: string;

  constructor(basePath: string = './segments') {
    this.basePath = basePath;
    // Directory creation is now lazy - only when needed
  }
  ...
  async createSegment(
    segmentId: string,
    type: SegmentType,
    initialSize: number = 16 * 1024 * 1024 // 16MB default
  ): Promise<MMapSegment> {
    const span = LensTracer.createChildSpan('create_segment', {
    ...
    try {
      // Ensure directory exists before creating segment
   ...
   */
  async openSegment(segmentId: string, readonly: boolean = false): Promise<MMapSegment> {
    const span = LensTracer.createChildSpan('open_segment', {
      'segment.id': segmentId,
  ...
  async writeToSegment(
    ...
    const span = LensTracer.createChildSpan('write_segment', {
      'segment.id': segmentId,
   ...
   */
  async readFromSegment(
    segmentId: string,
    offset: number,
    length: number
  ): Promise<Buffer> {
    const span = LensTracer.createChildSpan('read_segment', {
      'segment.id': segmentId,
      'segment.offset': offset,
   ...
   */
  async expandSegment(segmentId: string, additionalSize: number): Promise<void> {
    const span = LensTracer.createChildSpan('expand_segment', {
      'segment.id': segmentId,
    ...
    try {
      ...
      const limitMb = PRODUCTION_CONFIG.resources.shard_size_limit_mb;
  ...
  async closeSegment(segmentId: string): Promise<void> {
    const span = LensTracer.createChildSpan('close_segment', {
      'segment.id': segmentId,
    });
   ...
   */
  async getSegmentInfo(segmentId: string): Promise<Segment> {
    const files = fs.readdirSync(this.basePath);
    const segmentFile = files.find(f => f.startsWith(`${segmentId}.`) && f.endsWith('.seg'));
    ...
    const filePath = path.join(this.basePath, segmentFile);
    const stats = fs.statSync(filePath);
    const type = segmentFile.split('.')[1] as SegmentType;
   ...
   */
  async compactSegment(segmentId: string): Promise<void> {
    const span = LensTracer.createChildSpan('compact_segment', {
      'segment.id': segmentId,
    });
   ...
   */
  private writeHeader(fd: number, header: SegmentHeader): void {
    const headerBuffer = Buffer.alloc(64);
    let offset = 0;

    headerBuffer.writeUInt32LE(header.magic, offset); offset += 4;
    headerBuffer.writeUInt32LE(header.version, offset); offset += 4;
    headerBuffer.writeUInt32LE(this.typeToNumber(header.type), offset); offset += 4;
    headerBuffer.writeUInt32LE(header.size, offset); offset += 4;
    headerBuffer.writeUInt32LE(header.checksum, offset); offset += 4;
    headerBuffer.writeBigUInt64LE(BigInt(header.created_at), offset); offset += 8;
   ...
   * Read segment header
   */
  private readHeader(fd: number): SegmentHeader {
    const headerBuffer = Buffer.alloc(64);
    ...
    const type = this.numberToType(headerBuffer.readUInt32LE(offset)); offset += 4;
    const size = headerBuffer.readUInt32LE(offset); offset += 4;
    const checksum = headerBuffer.readUInt32LE(offset); offset += 4;
    const created_at = Number(headerBuffer.readBigUInt64LE(offset)); offset += 8;
   ...
   */
  private validateHeader(header: SegmentHeader): void {
   ...
   */
  private typeToNumber(type: SegmentType): number {
   ...
   */
  private numberToType(num: number): SegmentType {
