...
export class SemanticRerankEngine {
  ...
  private queryEmbeddingCache: Map<string, Float32Array> = new Map(); // query -> embedding
  ...
  // Phase 3: Configurable parameters for tuning
  private config: {
    nlThreshold?: number;
  ...
  async rerankCandidates(
    ...
    try {
      // Check if semantic reranking should be applied using current configuration
      const semanticConfig = {
        nlThreshold: this.config.nlThreshold,
      ...
      if (!shouldApplySemanticReranking(context.query, candidates.length, context.mode, semanticConfig)) {
        ...
        span.setAttributes({ 
          ...
          skipped: true,
          reason: reason,
          config_applied: JSON.stringify(semanticConfig)
        });
  ...
  /**
   * Update semantic configuration for Phase 3 parameter tuning
   */
  async updateConfig(config: {
    ...
    const span = LensTracer.createChildSpan('update_semantic_config', {
      'config.nl_threshold': config.nlThreshold || 0,
      'config.min_candidates': config.minCandidates || 0,
      'config.ef_search': config.efSearch || 0,
      'config.confidence_cutoff': config.confidenceCutoff || 0,
    });

    try {
      // Store configuration for use in query classification and reranking
      this.config = {
        ...this.config,
        ...config,
      };

      // Update HNSW index parameters if specified
      if (config.efSearch !== undefined && this.hnswIndex) {
        this.hnswIndex.max_connections = Math.min(config.efSearch, 128);
        console.log(`🔧 Updated HNSW efSearch to ${config.efSearch}`);
      }
  ...
  /**
   * Get or cache query embedding for performance
   */
