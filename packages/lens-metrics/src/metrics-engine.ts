...
export class LensMetricsEngine {
  ...
  private scorer: HierarchicalScorer;
  ...
  evaluateSystem(
    ...
    for (const queryData of systemResults.queries) {
      const metrics = this.scorer.scoreQuery(
        queryData.query,
        queryData.results,
        queryData.latency_ms
      );
    ...
    const validationReport = this.runValidationGates(
      systemResults.system_id,
      queryMetrics,
      validationGates
    );
    
    // Generate pool membership report
    const poolMembershipReport = this.generatePoolMembershipReport(
      systemResults,
      pooledQrels
    );
  ...
  buildPooledQrels(
    ...
    const pooledQrels: Map<string, PooledQrels> = new Map();
      ...
      for (const queryData of systemResult.queries) {
        ...
        for (const result of slaResults) {
          ...
          if (normalizedResult.line !== null && normalizedResult.col !== null) {
            const spanExists = pool.expected_spans.some(span => 
              span.repo === normalizedResult.repo &&
          ...
          // Add file
          const fileExists = pool.expected_files.some(file =>
            file.repo === normalizedResult.repo &&
  ...
  private runValidationGates(
    ...
    const medianHitsInPool = this.calculateMedian(
      queryMetrics.map(m => m.hits_in_pool)
    );
    ...
    if (medianHitsInPool <= gates.min_median_hits_in_pool) {
      ...
      report.errors.push(
        `GATE FAIL: median(hits_in_pool) = ${medianHitsInPool} <= ${gates.min_median_hits_in_pool}. ` +
    ...
    if (meanNdcg >= gates.max_perfect_score_threshold) {
      report.warnings.push(
        `WARN: mean(ndcg@10) = ${meanNdcg.toFixed(3)} >= ${gates.max_perfect_score_threshold}. ` +
    ...
    if (totalCredits > 0) {
      ...
      if (fileRatio > 0.8) {
        report.warnings.push(
          `WARN: ${(fileRatio * 100).toFixed(1)}% of credits from file-level matches. ` +
  ...
  private calculateAggregateMetrics(queryMetrics: QueryMetrics[]): AggregateMetrics {
