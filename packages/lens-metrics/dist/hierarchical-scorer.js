...
export class HierarchicalScorer {
     ...
     */
    scoreResults(query, results, expectedKeys) {
     ...
     */
    calculateMetrics(query, scoredResults, allResults, latencyMs) {
     ...
     */
    getTotalRelevantCount(query) {
        let total = 0;
        if (query.expected_spans)
            total += query.expected_spans.length;
        if (query.expected_symbols)
            total += query.expected_symbols.length;
